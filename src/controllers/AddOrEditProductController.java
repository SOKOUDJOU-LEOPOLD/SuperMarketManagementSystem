...
public class AddOrEditProductController implements Initializable {
    ...
    private HashTable<Category> categoryList  = new HashTable<>() ;
    ...
    public void initWindow(){
        ...
        list2 = new String[categoryList.getTableList().size()];
        for (int i = 0; i < list2.length; i++) {
            list2[i] = categoryList.getTableList().get(i).getName();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        categoryList = manager.getCategories();
        for (int i = 0; i < categoryList.getTableList().size(); i++) {
            categoryComboBox.getItems().add(categoryList.getTableList().get(i).getName());
